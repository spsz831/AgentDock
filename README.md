...
- `validate/export/install --json` 现已统一为版本化协议（`schemaVersion`、`generatedAt`、`toolVersion`、`command`、`success`、`data`、`errors`）
- `validate/export/install --json` 的 `errors` 为结构化数组：`[{ code, message }]`
...
## JSON Error Codes (validate/export/install)
